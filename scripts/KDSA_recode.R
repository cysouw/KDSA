...
LOC <- read.fwf("../sources/original/KD_MILI.DAT"
...
write.table( format(COOR, digits = 5, nsmall = 4)
			 , file = "../sandbox/KDSA_locations.txt"
			 , sep = "\t"
			 , quote = F
			 , col.names = NA
			 )
...
DATA <- read_fwf("../sources/original/KD_vekt_1995-11-26.dat"
				, fwf_widths( widths )
				, col_types = types
				)
...
tok <- tokenize(w, profile = "data/KDSA_profile.txt", regex = T, file = "sandbox/tokenized")
write.profile(tok$strings$tokenized, sep = " ", file = "sandbox/all_graphemes.txt")

# write output
write.table(WORDS
			, file = "sandbox/KDSA_words.txt"
			, sep="\t"
			, quote = FALSE
			, col.names = NA
			)
